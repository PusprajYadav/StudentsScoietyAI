...
export interface WhitebookShareState {
  open: boolean;
...
export function WhitebookShareSheet({
  shareState,
  setShareState,
  joinedCommunities,
  shareUrlFallback,
  ...
  shareState: WhitebookShareState;
  setShareState: Dispatch<SetStateAction<WhitebookShareState>>;
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/55 p-3 sm:items-center sm:p-6">
      <div className="surface-card max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-[28px] p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-app-muted">Share WhiteBook</p>
            ...
            <p className="mt-1 text-[11px] leading-5 text-app-muted sm:text-xs">
              Publish a snapshot of this WhiteBook to the feed so other students can open it, import it, and keep editing from their own notebook.
            </p>
          </div>
          ...
          {shareState.destination === "community" ? (
            ...
            <label className="grid gap-1 text-[11px] font-medium text-app-muted">
              ...
              <select
                ...
                disabled={shareState.loading || shareState.submitting}
              >
                {(["job", "study", "anonymous"] as const).map((category) => (
        ...
        <label className="mt-3 grid gap-1 text-[11px] font-medium text-app-muted">
          ...
          <input
            ...
            className="input-field"
            placeholder="My WhiteBook notes for this topic"
        ...
        <label className="mt-3 grid gap-1 text-[11px] font-medium text-app-muted">
          ...
          <textarea
            ...
            placeholder="Sharing my board for revision, feedback, or to help others build on it..."
            disabled={shareState.submitting}
          />
        </label>
        ...
        <div className="mt-3 rounded-[20px] border border-app-border bg-app-card p-3">
          <div className="mb-2 flex items-center gap-2 text-app-text">
            ...
            <p className="font-medium text-xs sm:text-sm">Editable WhiteBook snapshot</p>
          </div>
          <p className="text-[11px] leading-5 text-app-muted">
            Your post shares an editable WhiteBook snapshot. Anyone can open it and import their own local copy into WhiteBook. When live sync is available, the notebook also keeps a synced share link ready at <strong>{shareUrlFallback}</strong>.
          </p>
        </div>

        <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
          <p className="text-[11px] text-app-muted">
            {shareState.loading ? "Loading sharing options..." : "Share only what you want others to copy and build on."}
          </p>
          <button
            type="button"
            onClick={onSubmit}
