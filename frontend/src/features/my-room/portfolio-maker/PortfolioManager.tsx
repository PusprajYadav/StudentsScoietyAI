...
interface PortfolioManagerProps {
  ...
  importing: boolean;
  onCreate: (templateKey: PortfolioRecord["template_key"]) => Promise<void>;
  onDelete: (portfolio: PortfolioRecord) => Promise<void>;
  onImport: (file: File) => Promise<void>;
}

const templateShowcase: Record<
  PortfolioRecord["template_key"],
  {
    chips: string[];
...
> = {
  minimal_hero: {
    chips: ["Editorial", "Elegant", "Balanced"],
    previewClassName: "from-slate-950 via-slate-800 to-sky-500",
  },
  bold_cards: {
    chips: ["Bento", "Bold", "High Contrast"],
    previewClassName: "from-orange-950 via-slate-900 to-teal-400",
  },
  creative_timeline: {
    chips: ["Narrative", "Timeline", "Cinematic"],
    previewClassName: "from-violet-950 via-slate-900 to-amber-400",
  },
};
...
export function PortfolioManager({
  ...
  const remainingSlots = Math.max(0, portfolioLimit - portfolios.length);
  const authRedirectPath = buildAuthRedirectPath({ pathname: "/app/myroom/portfolio-maker" });
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[32px] border border-app-border bg-gradient-to-br from-brand/10 via-app-card to-app-secondary/70 p-4 shadow-[0_22px_64px_-36px_rgba(37,99,235,0.38)] dark:from-brand/20 dark:to-app-secondary/45 sm:p-6 lg:p-7">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(34,197,94,0.16),transparent_42%),radial-gradient(circle_at_bottom_right,rgba(59,130,246,0.18),transparent_36%)] dark:bg-[radial-gradient(circle_at_top_left,rgba(34,197,94,0.22),transparent_44%),radial-gradient(circle_at_bottom_right,rgba(96,165,250,0.2),transparent_38%)]" />
        ...
        <div className="relative grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-center">
          <div>
            ...
            <h1 className="mt-3 max-w-[14ch] font-display text-[1.9rem] font-bold leading-[0.98] tracking-tight text-app-text sm:text-[2.6rem]">
              Ship a real website.
            </h1>
            <p className="mt-2 max-w-lg text-sm leading-6 text-app-muted">
              Choose a template, edit sections, and publish a live portfolio link that looks great on mobile and desktop.
            </p>
              ...
              {isLoggedIn ? (
                <>
                  <div className="inline-flex items-center gap-2 rounded-full bg-brand px-4 py-2 text-sm font-semibold text-white">
                    ...
                    {portfolios.length} / {portfolioLimit} used
                  </div>
          ...
          <div className="rounded-[28px] border border-app-border bg-app-card/85 p-3.5 backdrop-blur">
            ...
            <div className="mt-3 grid gap-2.5">
              {[
                { label: "Templates", value: "3 ready", icon: LayoutTemplate },
                { label: "Coin cost", value: createCoinCostLabel, icon: FilePlus2 },
                { label: "Publish", value: "Live website", icon: Globe2 },
                { label: "Backup", value: "Export JSON", icon: FileJson },
              ].map((item) => (
                <div
                  key={item.label}
      ...
      <section className="surface-card rounded-[32px] p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-3">
          {portfolioTemplateOptions.map((template) => {
      ...
      <section className="surface-card rounded-[32px] p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-app-muted">Saved portfolios in one place.</p>
          </div>
        ...
        {!isLoggedIn ? (
          <div className="mt-6 rounded-[28px] border border-dashed border-app-border bg-app-secondary/35 px-5 py-10 text-center">
            <p className="font-display text-xl font-semibold text-app-text">Sign in to save portfolios.</p>
            <p className="mt-2 text-sm leading-7 text-app-muted">
              Public portfolio pages stay shareable, but editing lives inside your account.
            </p>
            <Link
              to={authRedirectPath}
          ...
          <div className="mt-5 grid gap-3 xl:grid-cols-2">
            {portfolios.map((portfolio) => {
              const showcase = templateShowcase[portfolio.template_key];
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span
                              className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${
                                portfolio.is_live ? "bg-emerald-500/15 text-emerald-700" : "bg-app-secondary text-app-muted"
                              }`}
                            >
                              {portfolio.is_live ? "Live" : "Draft"}
                            </span>
                          </div>
                          ...
                          <div className="mt-2 flex flex-wrap gap-1.5 text-[10px] font-semibold uppercase tracking-[0.08em]">
                            <span className="rounded-full bg-app-secondary px-2.5 py-1 text-app-text">
                              {resolveTemplateLabel(portfolio.template_key)}
                            </span>
                          </div>
                        </div>

                        <p className="rounded-full bg-app-secondary px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-app-muted">
                          {formatCompactUpdatedAt(portfolio.updated_at)}
                        </p>
                      </div>

                      <div
                        className={`mt-4 grid gap-2 sm:grid-cols-2 ${
                          portfolio.is_live ? "md:grid-cols-4" : "md:grid-cols-3"
                        }`}
                      >
                        <Link
                          to={`/app/myroom/portfolio-maker/edit/${portfolio.id}`}
                        ...
                        {portfolio.is_live ? (
                          <a
                            href={buildPublicPortfolioUrl(portfolio)}
                            target="_blank"
                            rel="noreferrer"
                        ...
                        {portfolio.is_live ? (
                          <button
                            type="button"
                            onClick={async () => {
                              await navigator.clipboard.writeText(buildPublicPortfolioUrl(portfolio));
                            }}
          ...
          <div className="mt-6 rounded-[28px] border border-dashed border-app-border bg-app-secondary/35 px-5 py-10 text-center">
            <p className="font-display text-xl font-semibold text-app-text">No portfolios yet.</p>
            <p className="mt-2 text-sm leading-7 text-app-muted">
              Pick a template above or import a JSON backup to start.
            </p>
          </div>
        )}
      </section>
