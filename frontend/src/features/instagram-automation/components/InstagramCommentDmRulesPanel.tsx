...
import { CheckCircle2, ChevronDown, ChevronUp, Clock3, PencilLine, Plus, Send, Trash2 } from "lucide-react";
import type { InstagramCommentDmRuleRow } from "../../../types/database";
import {
  buildInstagramMediaLabel,
  createEmptyInstagramCommentDmRuleDraft,
  formatInstagramRelativeTime,
  parseInstagramKeywordInput,
  stringifyInstagramKeywordInput,
  truncateInstagramText,
} from "../helpers";
import type { InstagramCommentDmRuleDraft, InstagramMediaItem } from "../types";

interface InstagramCommentDmRulesPanelProps {
  rules: InstagramCommentDmRuleRow[];
  media: InstagramMediaItem[];
  busySaving: boolean;
  deletingRuleId: string | null;
  onSave: (draft: InstagramCommentDmRuleDraft) => Promise<void>;
...
export function InstagramCommentDmRulesPanel({
  rules,
  media,
  busySaving,
  deletingRuleId,
  onSave,
  onDelete,
}: InstagramCommentDmRulesPanelProps) {
  const [draft, setDraft] = useState<InstagramCommentDmRuleDraft>(createEmptyInstagramCommentDmRuleDraft());
  const [formOpen, setFormOpen] = useState(rules.length === 0);

  useEffect(() => {
    if (draft.id && !rules.some((rule) => rule.id === draft.id)) {
      setDraft(createEmptyInstagramCommentDmRuleDraft());
    }
  }, [draft.id, rules]);
  ...
  const canSubmit = Boolean(draft.trigger_keyword.trim() && draft.comment_reply_text.trim() && draft.dm_reply_text.trim());
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)]">
      <section className="rounded-[28px] border border-app-border bg-app-card/90 p-4 shadow-[0_28px_64px_-42px_rgba(15,23,42,0.4)] backdrop-blur sm:rounded-[30px] sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h3 className="mt-1 font-display text-xl font-semibold text-app-text sm:text-2xl">Lead-flow combo</h3>
              <p className="mt-2 text-sm text-app-muted">Public comment reply plus private DM in one compact rule.</p>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <MetricPill icon={Send} label="Rules" value={rules.length.toString()} />
        ...
        {formOpen ? (
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-2 md:col-span-2">
                ...
                <input
                  ...
                  onChange={(event) => setDraft((current) => ({ ...current, media_id: event.target.value.trim() || null }))}
                  placeholder="Leave blank to match any post"
                />
              </label>

              <label className="space-y-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">Trigger</span>
                <input
                  className={inputClassName}
                  value={draft.trigger_keyword}
                  onChange={(event) => setDraft((current) => ({ ...current, trigger_keyword: event.target.value }))}
                  placeholder="link"
                />
              </label>

              <label className="space-y-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">Extra triggers</span>
                <input
                  className={inputClassName}
                  value={stringifyInstagramKeywordInput(draft.trigger_keyword_list)}
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      trigger_keyword_list: parseInstagramKeywordInput(event.target.value),
                    }))
                  }
                  placeholder="link, details, info"
                />
              </label>
              ...
              <label className="space-y-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      match_mode: event.target.value as InstagramCommentDmRuleDraft["match_mode"],
                    }))
                  }
                >
              ...
              <label className="space-y-2 md:col-span-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">Public reply</span>
                <textarea
                  className={`${inputClassName} min-h-[96px] resize-y`}
                  value={draft.comment_reply_text}
                  onChange={(event) => setDraft((current) => ({ ...current, comment_reply_text: event.target.value }))}
                  placeholder="Sending it in DM now."
                />
              </label>

              <label className="space-y-2 md:col-span-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">Public variants</span>
                <textarea
                  className={`${inputClassName} min-h-[88px] resize-y`}
                  value={stringifyInstagramKeywordInput(draft.comment_reply_variants)}
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      comment_reply_variants: parseInstagramKeywordInput(event.target.value),
                    }))
                  }
                  placeholder="Check your inbox, DM sent"
                />
              </label>

              <label className="space-y-2 md:col-span-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">DM reply</span>
                <textarea
                  className={`${inputClassName} min-h-[100px] resize-y`}
                  value={draft.dm_reply_text}
                  onChange={(event) => setDraft((current) => ({ ...current, dm_reply_text: event.target.value }))}
                  placeholder="Here is the link you asked for: https://..."
                />
              </label>

              <label className="space-y-2 md:col-span-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">DM variants</span>
                <textarea
                  className={`${inputClassName} min-h-[88px] resize-y`}
                  value={stringifyInstagramKeywordInput(draft.dm_reply_variants)}
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, dm_reply_variants: parseInstagramKeywordInput(event.target.value) }))
                  }
                  placeholder="Here's the link, Sharing details now"
                />
              </label>

              <label className="space-y-2 md:col-span-2">
                <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-app-muted">Else public reply</span>
                <textarea
                  className={`${inputClassName} min-h-[88px] resize-y`}
                  value={draft.fallback_comment_reply_text || ""}
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, fallback_comment_reply_text: event.target.value }))
                  }
                  placeholder="Optional public reply when the comment misses the trigger keywords"
                />
              </label>
              ...
              <label className="flex items-center justify-between rounded-[18px] border border-app-border bg-app-card px-4 py-3 md:col-span-2">
                <span>
                  ...
                  <span className="block text-xs text-app-muted">Enable both the public and DM steps</span>
                </span>
                <input
                  type="checkbox"
          ...
          >
            <div>
              <p className="text-sm font-semibold text-app-text">Create a new comment-to-DM rule</p>
      ...
      <section className="rounded-[28px] border border-app-border bg-app-card/90 p-4 shadow-[0_28px_64px_-42px_rgba(15,23,42,0.4)] backdrop-blur sm:rounded-[30px] sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 font-display text-xl font-semibold text-app-text sm:text-2xl">Lead playbooks</h3>
          </div>
        ...
        <div className="mt-4 space-y-3">
          {rules.length === 0 ? (
            <div className="rounded-[22px] border border-dashed border-app-border px-5 py-10 text-center text-sm text-app-muted">
              No comment-to-DM rules yet. Create one to turn public comments into guided inbox conversations.
            </div>
          ) : null}
            ...
            <article key={rule.id} className="rounded-[22px] border border-app-border/80 bg-app-secondary/70 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-3 text-sm font-semibold text-app-text">{truncateInstagramText(rule.comment_reply_text, 82)}</p>
                  <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-app-muted">
                    <span>DM: {truncateInstagramText(rule.dm_reply_text, 72)}</span>
                    ...
                    {rule.fallback_comment_reply_text ? <span>Else: {truncateInstagramText(rule.fallback_comment_reply_text, 38)}</span> : null}
